...
var badGlobalVar = "This should not be global";
...
async function createImageBufferFromAscii(asciiText, aspectRatio = 1.91) {
  ...
  const svgImage = `
    <svg width="${width}" height="${height}" xmlns="<http://www.w3.org/2000/svg>">
      ...
      <text x="10" y="${padding}" class="text-content">
        ${lines.map((line, index) => `<tspan x="10" dy="${index === 0 ? '0' : lineHeight}">${line}</tspan>`).join('')}
      </text>
    </svg>
  `;
...
export async function runCliCommand(inputs) {
  ...
  var web3PrivateKey = process.env.WEB3_PRIVATE_KEY;
